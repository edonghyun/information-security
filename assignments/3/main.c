	...
	*/
	...
	while (1)
	{
		...
		*/
		...
		HexaToBinary(Test, KeyT, 8); // Key T : 32비트 키
		HexaToBinary(plaintext, PlainT, 16); // PlainT  64비트 텍스트

		FeistelCipher(KeyT, PlainT, result); // result : 64비트 암호문 
		BinaryToHexa(result, FP, 64); // FP : 16비트 암호문

		if (strcmp(FP, answer) == 0) {  // FP 와 주어진 암호문 비교
		printf("Key is .. : %s\n", KeyT);
